...
/** Emisoras personalizadas del usuario (Bearer). Paridad con `CustomStations.tsx` web. */
interface CustomStationsApi {
    ...
    /**
     * Crea una emisora personalizada (`{name 1–256, url http(s)}` → `{station}`).
     * El servidor responde 422 con `details` por campo si la validación falla.
     */
