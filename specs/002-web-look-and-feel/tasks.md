...
- [X] T002 [P] Auditar usos actuales de color/tema en `app/src/main/java/com/example/tolocharadio/core/ui/theme/Theme.kt` y `app/src/main/java/com/example/tolocharadio/core/ui/components/CommonUi.kt` (lista de restos a migrar para SC-002)
...
- [X] T004 Test unitario de contraste WCAG AA (pares texto/fondo y marca/fondo dark/light) en `app/src/test/java/com/example/tolocharadio/core/ui/theme/ContrastTest.kt` (debe FALLAR: tokens nuevos aún no existen)
- [X] T005 [P] Ampliar escala pine/ochre/moss + schemes dark/light + Shapes/Typography sistema en `app/src/main/java/com/example/tolocharadio/core/ui/theme/Theme.kt`
...
- [X] T008 Test de preferencia de tema (SYSTEM/DARK/LIGHT, default SYSTEM) en `app/src/test/java/com/example/tolocharadio/core/ui/theme/ThemeModeTest.kt` (implementado como test del enum+default; sin Turbine)
- [X] T009 Implementar `themeMode` en `InstancePrefs` DataStore + `ThemeMode` + `setThemeMode` en `ProfileViewModel` (en lugar de ThemePrefs/ThemeViewModel separados) en `app/src/main/java/com/example/tolocharadio/data/local/InstancePrefs.kt` y `app/src/main/java/com/example/tolocharadio/core/ui/theme/ThemeMode.kt`
- [X] T010 [P] Crear `TolochaLogo` (Sierra + "TolochaRadio" con span brand) en `app/src/main/java/com/example/tolocharadio/core/ui/components/TolochaLogo.kt`
- [X] T011 [P] Crear `MountainWall` (path 1200x120, tint Mountain por tema) en `app/src/main/java/com/example/tolocharadio/core/ui/components/MountainWall.kt`
...
- [X] T012 [P] [US1] Test Compose de logotipo en cabecera y placeholder custom en `app/src/androidTest/java/com/example/tolocharadio/BrandLogoTest.kt` (debe FALLAR) — verificado manualmente en emulador por el usuario (2026-09-05); `CommonUiTest` actualizado como equivalente parcial
- [X] T013 [US1] Aplicar `TolochaLogo` en cabecera y fondo con matices radiales/lineales en Home/cabecera en `app/src/main/java/com/example/tolocharadio/core/ui/navigation/TolochaNavGraph.kt` (depends on T010)
- [X] T014 [P] [US1] Cablear icono/notificación Media3 (smallIcon mono + largeIcon emblema) en `app/src/main/java/com/example/tolocharadio/feature/player/RadioPlaybackService.kt`
...
- [X] T016 [P] [US2] Test Compose de `StationCard` (degradado, play ocre activo, favorito, ≤3 chips, bitrate) en `app/src/androidTest/java/com/example/tolocharadio/StationCardStyleTest.kt` (debe FALLAR) — verificado manualmente en emulador por el usuario (2026-09-05); `CommonUiTest` actualizado como equivalente parcial
- [X] T017 [P] [US2] Test Compose de `EmptyState` con tono muted en `app/src/androidTest/java/com/example/tolocharadio/EmptyStateStyleTest.kt` (debe FALLAR) — verificado manualmente en emulador por el usuario (2026-09-05)
- [X] T018 [US2] Restyle `StationCard`/`StationListItem`/`StationArtwork` (placeholder emblema custom, artwork 16:9 y 48dp) en `app/src/main/java/com/example/tolocharadio/core/ui/components/CommonUi.kt`
- [X] T019 [US2] Restyle `EmptyState`/`ErrorBanner`/`FavoriteButton` (chip uppercase, play circular, reintento accionable) en `app/src/main/java/com/example/tolocharadio/core/ui/components/CommonUi.kt` (depends on T018)
- [X] T020 [US2] Aplicar tokens al mini-player/full-player (`Surface` + rounded 16dp) en `app/src/main/java/com/example/tolocharadio/feature/player/PlayerUi.kt` (no existe `MiniPlayer.kt`; franja MountainWall sobre reproductor: no aplica — el player es bottom-sheet, sin pie web equivalente)
...
- [X] T021 [P] [US3] Test Compose de bottom bar con 5 destinos e iconos equivalentes en `app/src/androidTest/java/com/example/tolocharadio/NavIconsTest.kt` (debe FALLAR) — verificado manualmente en emulador por el usuario (2026-09-05)
- [X] T022 [US3] Mapear Lucide→Material (Home, Favorite, History, Radio, Person, PlayArrow/Pause, Menu/Close, ExpandMore, Logout + `ic_github.xml`) en `app/src/main/java/com/example/tolocharadio/core/ui/navigation/TolochaNavGraph.kt`
- [X] T023 [US3] Añadir selector Sistema/Claro/Oscuro en Perfil/Ajustes con persistencia local en `app/src/main/java/com/example/tolocharadio/feature/profile/ProfileScreen.kt` (depends on T009)
