...
- [X] T032 [US2] Verificar Cast con la misma fuente autenticada (`app/src/main/java/com/izquierdojl/tolocharadio/cast/CastPlayerManager.kt` + `feature/player/PlayerDataSourceFactory.kt`) incl. subrecursos HLS
  *(Corrección converge 2026-09-20: la verificación descubrió que Cast NO puede enviar `Authorization` (el receptor no soporta cabeceras → 401, bug 0026); la excepción está registrada en la spec 0021 §9. La vía local (proxy + subrecursos HLS) sí usa `Bearer` — `PlayerDataSourceFactory.kt`.)*
