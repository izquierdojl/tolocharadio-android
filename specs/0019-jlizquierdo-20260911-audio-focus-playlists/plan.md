...
**Deuda registrada (gobernanza)**: [issue #4](https://github.com/izquierdojl/tolocharadio-android/issues/4) — "Resolver listas m3u/m3u8/pls en el proxy para restaurar historial"; revisión **2026-10-09** (máx. 2 sprints).

**Estado (2026-09-12)**: **excepción retirada**. El backend resolvió las listas y el HLS en el proxy (`resolve-playlist-proxy`) y la spec [0021](../0021-jlizquierdo-20260912-proxy-only-playback/spec.md) devuelve la app a la reproducción por el proxy autenticado, restaurando el historial server-side. Las dos filas de Complexity Tracking anteriores dejan de aplicar.
