...
**Origen**: `/speckit.converge` 2026-09-20 — 6 FR, 2 historias y 5 principios revisados contra el código. Filtros combinados, autocompletado con catálogo, modo degradado, spinner y `ExploreViewModelsTest` conformes; dos desviaciones menores de texto/documentación.

- [X] T018 Alinear la etiqueta del botón de limpieza de filtros en `app/src/main/java/com/izquierdojl/tolocharadio/feature/explore/ExploreScreen.kt` (línea 174, `Text("Limpiar")`) con la spec/quickstart ("Limpiar filtros"), o anotar en `specs/013-explore-filters/spec.md` FR-005 y `quickstart.md` la forma corta si se prefiere conservarla per FR-005 (partial) — LOW
- [X] T019 Anotar en `specs/013-explore-filters/spec.md` (FR-001–FR-003) y `research.md` (decisión del combo) que el popup `ExposedDropdownMenuBox` se sustituyó por lista inline dentro del bottom sheet por el bug `filter-dropdown-overlap` (`.specify/bugs/filter-dropdown-overlap/fix.md`), manteniendo autocompletado y entrada manual per plan: FilterComboBox (partial) — LOW
