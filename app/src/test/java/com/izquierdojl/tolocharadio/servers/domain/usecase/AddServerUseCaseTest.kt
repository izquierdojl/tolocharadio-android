...
class AddServerUseCaseTest {
    ...
    @Test
    fun `invoke returns error when alias is blank`() = runTest {
        val result = useCase("https://radio.example.com", "")
        assertTrue(result is ApiResult.Err)
    }

    @Test
    fun `invoke returns error when repository add fails`() = runTest {
        coEvery { repository.add(any(), any(), any(), any(), any()) } returns ApiResult.Err(
    ...
    @Test
    fun `invoke returns server on success`() = runTest {
        val server = SavedServerEntity(
            id = "1",
    ...
    @Test
    fun `invoke propaga email y password al repositorio (FR-005)`() = runTest {
        coEvery { repository.add(any(), any(), any(), any(), any()) } returns ApiResult.Err(
