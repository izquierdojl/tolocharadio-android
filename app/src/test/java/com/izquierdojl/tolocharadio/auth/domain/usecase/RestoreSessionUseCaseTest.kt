...
class RestoreSessionUseCaseTest {
    ...
    @Test
    fun `invoke returns current state when not Loading`() =
        runTest {
    ...
    @Test
    fun `invoke returns Unauthenticated when no refresh token`() =
        runTest {
    ...
    @Test
    fun `invoke returns Authenticated on successful refresh`() =
        runTest {
            val user = UserDto(id = 1, email = "test@test.com", name = "Test", theme = ThemeDto.DARK, createdAt = 0L)
            every { session.authState.value } returnsMany
                listOf(
                    AuthState.Loading,
    ...
    @Test
    fun `invoke returns Unauthenticated on failed refresh`() =
        runTest {
            every { session.authState.value } returnsMany
                listOf(
                    AuthState.Loading,
            ...
            coEvery { authRepo.refreshNow("invalid-refresh") } returns
                ApiResult.Err(
    ...
    @Test
    fun `sin refresh pero con credenciales guardadas re-autentica (FR-006)`() =
        runTest {
            val user = UserDto(id = 1, email = "test@test.com", name = "Test", theme = ThemeDto.DARK, createdAt = 0L)
            every { session.authState.value } returnsMany
                listOf(
    ...
    @Test
    fun `sin refresh ni credenciales pide login`() =
        runTest {
            every { session.authState.value } returnsMany
                listOf(
                    AuthState.Loading,
    ...
    @Test
    fun `refresh revocado re-autentica con password cifrado (FR-006b)`() =
        runTest {
            val user = UserDto(id = 1, email = "test@test.com", name = "Test", theme = ThemeDto.DARK, createdAt = 0L)
            every { session.authState.value } returnsMany
                listOf(
            ...
            coEvery { authRepo.refreshNow("revocado") } returns
                ApiResult.Err(
    ...
    @Test
    fun `refresh revocado sin password guardado pide login`() =
        runTest {
            every { session.authState.value } returnsMany
                listOf(
                    AuthState.Loading,
