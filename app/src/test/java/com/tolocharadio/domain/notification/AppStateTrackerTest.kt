...
class AppStateTrackerTest {
    ...
    @Test
    fun `initial state should be NOT_RUNNING`() = runTest {
        assertEquals(AppState.NOT_RUNNING, appStateTracker.currentState.value)
    }
    
    @Test
    fun `updating state to FOREGROUND should change current state`() = runTest {
        appStateTracker.updateState(AppState.FOREGROUND)
        assertEquals(AppState.FOREGROUND, appStateTracker.currentState.value)
    }
    
    @Test
    fun `updating state to BACKGROUND should change current state`() = runTest {
        appStateTracker.updateState(AppState.BACKGROUND)
        assertEquals(AppState.BACKGROUND, appStateTracker.currentState.value)
    }
    
    @Test
    fun `isForeground should return true when state is FOREGROUND`() = runTest {
        appStateTracker.updateState(AppState.FOREGROUND)
        assertTrue(appStateTracker.isForeground())
    }
    
    @Test
    fun `isForeground should return false when state is BACKGROUND`() = runTest {
        appStateTracker.updateState(AppState.BACKGROUND)
        assertFalse(appStateTracker.isForeground())
    }
    
    @Test
    fun `isBackground should return true when state is BACKGROUND`() = runTest {
        appStateTracker.updateState(AppState.BACKGROUND)
        assertTrue(appStateTracker.isBackground())
    }
    
    @Test
    fun `isBackground should return false when state is FOREGROUND`() = runTest {
        appStateTracker.updateState(AppState.FOREGROUND)
        assertFalse(appStateTracker.isBackground())
    }
    
    @Test
    fun `isNotRunning should return true when state is NOT_RUNNING`() = runTest {
        assertTrue(appStateTracker.isNotRunning())
    }
    
    @Test
    fun `isNotRunning should return false when state is FOREGROUND`() = runTest {
        appStateTracker.updateState(AppState.FOREGROUND)
        assertFalse(appStateTracker.isNotRunning())
    }
    
    @Test
    fun `state should be updated correctly through multiple changes`() = runTest {
        appStateTracker.updateState(AppState.FOREGROUND)
        assertEquals(AppState.FOREGROUND, appStateTracker.currentState.value)
